...
import { OrganizationStructure, JobFamily } from "@/types/organization";
...
// Create node types with processing status
const createNodeTypes = (processingStatus?: any): NodeTypes => ({
  industry: IndustryNode,
  sub_entity: SubEntityNode,
  job_family: JobFamilyNode,
  nested_job_family: NestedJobFamilyNode,
  job_role: (props: any) => (
    <JobRoleNode {...props} processingStatus={processingStatus} />
  ),
});
...
const FlowEditorContent: React.FC<OrganizationFlowEditorProps> = ({
  ...
  const { setIsDirty } = useOrganization();
  ...
  const convertToFlowData = useCallback((data: OrganizationStructure) => {
    ...
    const calculateTotalHeight = (families: JobFamily[]): number => {
      return families.reduce((total, family) => {
        ...
        const subFamiliesHeight =
          family.sub_job_families.length > 0
            ? calculateTotalHeight(family.sub_job_families)
            : 0;
        const rolesHeight = family.job_roles.length * VERTICAL_SPACING;
    ...
    const processJobFamily = (
      ...
      if (jobFamily.sub_job_families.length > 0) {
  ...
  return (
    <div className="h-[600px] w-full border border-gray-200 rounded-lg relative bg-white shadow-sm">
      <ReactFlow
        ...
        nodeTypes={createNodeTypes(processingStatus)}
        edgeTypes={edgeTypes}
        fitView
        ...
        >
          <SheetHeader className="pb-6 border-b border-gray-100">
            ...
            <SheetDescription className="text-sm text-gray-600 mt-2">
              Edit the details of this node in the organization structure.
            </SheetDescription>
          </SheetHeader>
          <div className="p-6 space-y-6">
            ...
            {(editingNode?.type === "job_family" ||
              editingNode?.type === "nested_job_family") && (
              <div className="space-y-2">
